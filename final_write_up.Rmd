...
top10Log <- subset(pgaFinal, select = c(year, player_name, top_10_prob))
#View(pgaFinal)
```
...
hist(new$masters_top_10,  main = "Histogram of 10 top finish")
...
playing in the 2022 masters in total. But even though we have a large
confidence interval of players, this svm model predicted every single player
in the top 10 besides Corey Conners and Charl Schwartzel.
...
model. From this polynomial svm with 26 players, it predicted 3 of the top 10
players: Rory Mcllroy, Will Zalatoris, and Shane Lowry.




So we can conclude that the linear svm model worked best for test 2021 
data, but when we fed it new data to predict this coming masters, it did not do
well. When we passed in new 2022 data, we saw the polynomial model preform 
...
accurate as we thought it would be. Overall, we can conclude that it is not
easy to predict sporting events. We consider our model to be decent when we only
were able to precinct 4 players in the top ten. There is lots of noise and events
that can alter the outcome of sporting events, which makes it extremely difficult
