...
Creating Regression Decision Tree to Predict Total Score

```{r}

#data.train

# Dropping year column from test data set
#data.test
# data.test <- data.test[, -1]
# View(data.test)

dc.data_train <- data.train
dc.data_test <- data.test

dc.data_test <- subset(dc.data_test, select = -c(year))
...
# tree.pga = tree(total_score ~ driving_distance + sg_putt +
#                 ranking + rounds + gir + top_10 + putts_round, data.train)
tree.pga = tree(total_score ~., data.train)
summary(tree.pga)


plot(tree.pga)
text(tree.pga, pretty = 0, cex=.75)

```
Getting MSE for first trial of tree
```{r}
predVal <- predict(tree.pga, newdata = data.test)
actualVal <- data.test$total_score

MSE <- mean((predVal - actualVal)^2)
MSE
```
Using cross validation to find optimal level of tree complexity
```{r}
cv.tree_pga = cv.tree(tree.pga)
plot(cv.tree_pga$size, cv.tree_pga$dev, type='b')
```
After using cross-validation, we can determine that the optimal level of tree 
complexity for pruning is 3.


Pruning the tree
```{r}
prune.tree_pga <- prune.tree(tree.pga, best = 4)
plot(prune.tree_pga)
text(prune.tree_pga, pretty = 0, cex=.75)
summary(prune.tree_pga)
```
...
knn.data_train <- data.train
knn.data_train_final <- subset(knn.data_train, select = c(driving_distance, sg_putt, ranking, rounds, gir, top_10, putts_round, total_score))

knn.data_test <- data.test

knn.data_test <- subset(knn.data_test, select = -c(year))
knn.data_test_final <- subset(knn.data_test, select = c(driving_distance, sg_putt, ranking, rounds, gir, top_10, putts_round, total_score))

knn.data_train_final[,1:7] <- scale(knn.data_train_final[,1:7])

```

Players who had this type of skill were clustered together, using clustering.
...
# svm.data_test$masters_top_10 <- as.factor(svm.data_test$masters_top_10)
# svm.data_train$masters_top_10 <- as.factor(svm.data_train$masters_top_10)
